...
description: 'Coding agents are blind to animation. They read your test logs, watch them go green, and never see that the transition looks broken. Here’s a trick that gives an agent motion perception: capture the animation as a timed burst, tile it into one labeled contact sheet, and hand it back as a single image.'
pubDate: '2026-06-03'
...
There’s a blind spot in coding with AI agents that took me a while to name: **agents can’t see motion.**
...
A browser screenshot has a floor around 40 to 50 ms, so a 250 ms transition gives you three usable frames: start, one smear, end. The fix is to slow the animation down before you capture, overriding the engine's global time scale (GSAP's `timeScale`, or `playbackRate` on a `<video>`) by, say, 6×. Same flip, now sampled across the whole arc:
